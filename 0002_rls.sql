...
begin
  ...
  return coalesce(exists (
    ...
    where us.user_id = p_actor_user_id
      ...
      and (us.contest_id is null or us.contest_id = p_contest_id)
      and (
        us.target_user_id = p_target_user_id
        or (
          us.target_user_id is null
          and (
            us.unit_id is null
            or (
              v_target_unit_id is not null
...
begin
  ...
  if p_file_size_kb is null or p_file_size_kb <= 0 or p_file_size_kb > v_contest.max_file_size_kb then
    raise exception 'Invalid file size';
  end if;

  if lower(p_file_type) = 'pdf' and not v_contest.allow_pdf_evidence then
  ...
  if v_existing_submission.id is not null and not v_contest.allow_resubmission then
    raise exception 'Resubmission is disabled for this contest';
  end if;
